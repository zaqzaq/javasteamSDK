...
val currentPlatformNativeJar = tasks.register<Jar>("currentPlatformNativeJar") {
   doFirst {
      logger.error("Should create jar $archiveFile with:")
...
library {
   ...
   binaries.configureEach {
      ...
      logger
              .error("Looping binary name=${this.name}, compileTask=${binaryCompileTask.name}, linkTask=${binaryLinkTask.name}, linkTask.enabled=${binaryLinkTask.isEnabled}")
            ...
            macOsDylibLipo.configure {
               ...
               binaryLinkTask.outputs.files.files
                       ...
                       .ifPresent({ binaryFile ->
                                     ...
                                     logger.error("Found macOs binary file ${binaryFile}, adding to lipo")
                                     args("-arch")
         ...
         currentPlatformNativeJar.configure {
            ...
            if (targetMachine.operatingSystemFamily.isMacOs) {
               ...
               binaryLinkTask.outputs.files.forEach {
                  logger.error("binaryLink output = $it")
